...
export default async function ExpenseDetailsPage({ params }: ExpenseDetailsPageProps) {
  ...
  const paramsUser = await api.user.byId.query({ userId: params.userId });
  if (!paramsUser) {
    redirect('/');
  }
  ...
  const credits = await api.expense.between
    .query({
      payerId: user.id,
      debtorId: paramsUser.id,
    })
    .then((expenses) =>
      expenses.map((expense) => {
        const expenseDebt = expense.debts.find((expenseDebt) => expenseDebt.debtorId === paramsUser.id);
        const expenseAmount =
