...
const expensePayListSchema = z
  .object({ debts: z.array(z.string()) })
  .refine((values) => values.debts.length !== 0, {
...
export function ExpensePaymentSettleForm({
  paramUser,
  currentUser,
  debts,
}: ExpensePaymentSettleFormProps) {
  ...
  const payer = currentUserTotalAmount.greaterThan(paramUserTotalAmount)
    ? paramUser
    : currentUser;
  const debtor = currentUserTotalAmount.greaterThan(paramUserTotalAmount)
    ? currentUser
    : paramUser;
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSettleExpenseDebts)}>
          ...
          render={() => (
            <FormItem>
              <div className="mb-4">
                ...
                <FormDescription>
                  Zaznacz te które chcesz rozliczyć
                </FormDescription>
              </div>
              {debts.map((item) => (
                <FormField
                    ...
                    return (
                      <FormItem className="p-4 border space-y-2 rounded-md">
                        ...
                        <div className="grid grid-cols-5 place-items-center">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {(
                              Number(item.amount) - Number(item.settled)
                            ).toFixed(2)}
                            zł
                          </div>
                          <ChevronRight />
        ...
        <div className="mt-6">
          ...
          <div className="sticky bottom-16 flex items-center justify-end gap-4 mt-6">
            <Button
              type="button"
              size="icon"
              onClick={() =>
